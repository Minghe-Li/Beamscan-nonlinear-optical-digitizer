function GalvoResonantCountingLastFrame(AcqParameters,ImageData,Channel,LiveImagePlot,LastBufferInAcquisition)
...
if ishandle(LiveImagePlot.FigureHandle)
    if strfind(AcqParameters.FirstChannelToDisplay,[char(64+Channel),'-COUNT']) == 1
        Pol = str2double(AcqParameters.FirstChannelToDisplay(12:end));
    ...
    if AcqParameters.NumChannelsToDisplay == 2
        if strfind(AcqParameters.SecondChannelToDisplay,[char(64+Channel),'-COUNT']) == 1
            Pol = str2double(AcqParameters.SecondChannelToDisplay(12:end)); 
            if Pol <= AcqParameters.TotalCycledPolarizations
...
if AcqParameters.SaveProcessedData
    for Pol = 1:AcqParameters.TotalCycledPolarizations
        fwrite(AcqParameters.ProcessedDataChanPolAnalysisFID(Channel,Pol,2),FinalImage(:,:,Pol,:), 'float32');
    end
end    