...
AcqParameters.FilePath = 'D:\James\NOSE\2020\02-19-20\OnlyHWP-NoPolarizer\';  %If this path is not in purple, something is wrong!
...
AcqParameters.NumberOfFramesPerImage =  128;
...
%Channel B   
CaptureChannelB = 1;
ChannelBCount = 1;     ChannelBThreshold = -3;     ChannelBLambda = 0;
...
OverrideMeasuredGalvoResMirrorPhase = 1;           ManualGalvoResMirrorPhase = 2050; %2020 %-2550 %-2589
...
if AcqParameters.MultipleImagingSessions == 1;
    ...
    for SessionNum = 1:numOfPicsInSequence
        ...
        if numOfPicsInSequence > 1
            ...
            serialOne=serial('COM7', 'BaudRate', 9600, 'Terminator','CR/LF');
            fopen(serialOne);
       ...
       if Rotation_Start_Point<Rotation_End_Point
           ...
           Rotation_End_Point = tempa;
            ...
            for i = Rotation_Start_Point:Rotation_Inteval:Rotation_End_Point
                ...
                if translate
                    movementParamaters = strcat('GR,',num2str(0),',',num2str(trans_value));
                    accelParameters = strcat('SRF,4')
                    mkdir(AcqParameters.FilePath)
                    serialOne=serial('COM7', 'BaudRate', 9600, 'Terminator','CR/LF');
                    fopen(serialOne);
                    fprintf(serialOne,accelParameters)
                    fprintf(serialOne,movementParamaters);
                    fclose(serialOne);
                    pause(Pause_Time)
                    acquireData(boardCount, boardHandleArray,AcqParameters)
                    ...
                    %%
%                     movementParamaters = strcat(num2str(Rotation_Stage_Position2),'PA',num2str(0+Deg*2));
%                     fprintf(serialOne,movementParamaters);
%                     fprintf('\nMoving Rotation_Stage to %u Degree\n', Deg);
                    %%
                    fclose(serialOne);
                    if i ~= Rotation_Start_Point
                        pause(Pause_Time)
                    end
                    acquireData(boardCount, boardHandleArray,AcqParameters)
                end
