...
AcqParameters.FilePath = 'F:\Alex\053018\ureaScan\';  %If this path is not in purple, something is wrong!
...
%If you wish to do new and experimental measurements, you likely wish to do a unique analysis that is not currently implemented. This is the option
...
%Indicate if multiple imaging sessions will be automatically performed. Set to 0 for normal operation (a single imaging session), or 1 acquire multiple images. 

AcqParameters.MultipleImagingSessions = 1;
sequenceTimeDelay = 5; %sec
TotalTime = 1000000000000000; %min

% The above parameters are used if Use_ExcelFilePath == 0. This setting will not allow for movement of the sample stage. 
% Most of this data will override previously entered parameters. Prior stage will not be used on this setting. Use of the prior stage can be
% activated in the settings lower down
...
AcqParameters.NumberOfFramesPerImage = 3;
...
AcqParameters.NumberOfImages =  1; %5 frames per image? -Video rate may be too much for 30 minute DSC Run
%% Channels To Acquire%%%%%%%%%
...
AcqParameters.FlipCountingPolarity = 0;
%Channel A     
CaptureChannelA = 0;
ChannelACount = 0;     ChannelAThreshold = 0.5;     ChannelALambda = 0;
ChannelAAvg = 0;       ChannelAHybridAvgCount = 0;
                                             
%Channel B   
CaptureChannelB = 1;
ChannelBCount = 0;     ChannelBThreshold = -1;     ChannelBLambda = 0;
...
%Up to two images can be chosen to be displayiyed in realtime during acquisition. If input polarization 
...
AcqParameters.NumChannelsToDisplay = 1;
    ...
    AcqParameters.FirstChannelToDisplay = 'B-Avg-pol1'; % A-Count-Pol1,B-Avg-Pol2, etc.
    AcqParameters.SecondChannelToDisplay = 'A-Avg-Pol1'; 
...
AcqParameters.ShotsPerMirrorPeriodGalvoRes = 4535*2*1;% 4535  5112*2*4; % Same value as in resonant mirror control box software, (off by factor of 2)
...
%phase from a portion of a frame acquisition, succh as canceling an acquisition early, may trigger this scenario.
OverrideMeasuredGalvoResMirrorPhase = 1;           ManualGalvoResMirrorPhase = 2050;%2020*1; %-2290 %-2550 %-2589
...
ComPort = 'COM10'; % 'COM4', 'COM3', etc, depends on which USB plug you have it in.
...
AcqParameters.LissaJous = 0; %Indicates that this was a LissaJous scanned image
    ...
    AcqParameters.ShotsPerFastMirrorPeriod = 4540*1*2; %2626
    AcqParameters.ShotsPerSlowMirrorPeriod = 104420*1*2; %2965; 2964
...
OverrideMeasuredLissajousMirrorPhase = 1;  ManualLissajousFastMirrorPhase = 2090*1;  ManualLissajousSlowMirrorPhase = 14000*1;
...
if AcqParameters.MultipleImagingSessions == 1;
    ...
    if Use_ExcelFilePath == 0;
        numOfPicsInSequence = (TotalTime*60) / sequenceTimeDelay;
        ...
        for SessionNum = 1:numOfPicsInSequence;
            ...
            if numOfPicsInSequence > 1
                ...
                %If Res mirror is unstable, this will let you correct it
                %every image
%                 ManualGalvoResMirrorPhase = str2double(inputdlg());     %Alex added these 2 lines
                close all                                               %Alex added these 2 lines
            end
