...
AcqParameters.FilePath = 'D:\James\NOSE\2018\11-15-18\Collagen\EOM_110mV_60sIntegration2\';  %If this path is not in purple, something is wrong!
...
AcqParameters.SaveProcessedData = 1;

Auto_Rotation = 1;
Rotation_Stage_Port = 'COM7';
Rotation_Stage_Position = 1; %QWP; inner loop
Rotation_Start_Point = 0; 
Rotation_End_Point = Rotation_Start_Point+45; 
Rotation_Inteval = 22.5;
Pause_Time = 12; %In sec;

Rotation_Stage_Port2 = 'COM7'; %Polarizer; outer loop
Rotation_Stage_Position2 = 2;
Rotation_Start_Point2 = 0; 
Rotation_End_Point2 = Rotation_Start_Point2+135; 
Rotation_Inteval2 = 45;
Pause_Time2 = 25; %In sec;

Rotation_Stage_Position3 = 3;
Rotation_Start_Point3 = 0; 
Rotation_End_Point3 = Rotation_Start_Point2+60; 
Rotation_Inteval3 = 3;
Pause_Time3 = 1; %In sec;
...
AcqParameters.BufferSize = 0.5; % Units in gigabytes
...
AcqParameters.TotalCycledPolarizations = 10;
...
AcqParameters.NumberOfFramesPerImage =  16*60;
...
AcqParameters.NumberOfImages =  1;
%% Channels To Acquire%%%%%%%%%
...
AcqParameters.FlipCountingPolarity = false;
%Channel A     
CaptureChannelA = 1;
ChannelACount = 0;     ChannelAThreshold = 0.5;     ChannelALambda = 0;
ChannelAAvg = 1;       ChannelAHybridAvgCount = 0;
                                             
%Channel B   
CaptureChannelB = 1;
ChannelBCount = 1;     ChannelBThreshold = -2;     ChannelBLambda = 0;
ChannelBAvg = 1;       ChannelBHybridAvgCount = 0;

%Channel C    
CaptureChannelC = 0;
ChannelCCount = 0;      ChannelCThreshold = -0.5;     ChannelCLambda = 0;
ChannelCAvg = 0;       ChannelCHybridAvgCount = 0;

%Channel D 
CaptureChannelD = 0;
ChannelDCount = 0;     ChannelDThreshold = -0.5;       ChannelDLambda = 0;
ChannelDAvg = 0;       ChannelDHybridAvgCount = 0;
...
%Setting this value to 0 will prevent 1the realtime display window from showing. 
AcqParameters.NumChannelsToDisplay = 0;
    ...
    AcqParameters.SecondChannelToDisplay = 'B-avg-Pol1'; 
    ...
    InvChanA = 0;       InvChanB = 0;       InvChanC = 0;       InvChanD = 0;
...
AcqParameters.CountSaturationAlert = 1;
...
AcqParameters.CheckForClippingEveryNBuffers = 100000;
...
AcqParameters.ShotsPerMirrorPeriodGalvoRes = 4535*2*1;%5112*2*4; % Same value as in resonant mirror control box software, (off by factor of 2)
             % || 5153 for 1 micron; 5130 for 800 nm; 51 22 for 800 nm; ||
...
OverrideMeasuredGalvoResMirrorPhase = 1;           ManualGalvoResMirrorPhase = 2040; %-2290 %-2550 %-2589
...
ComPort = 'COM3'; % 'COM4', 'COM3', etc, depends on which USB plug you have it in.
...
SequenceExcelFilePath = 'F:\GeigerLab\Mona\2015.2.20\ExampleSequence.xlsx';
...
AcqParameters.LissaJous = 0; %Indicates that this was a LissaJous scanned image
    ...
    AcqParameters.ShotsPerFastMirrorPeriod = 2624*1*2; %2626
    AcqParameters.ShotsPerSlowMirrorPeriod = 2964*1*2; %2965
...
%for absent signal or weak signal, the algorithm chooses to not update phase instead of providing an uncertain measurement. Measuring
%phase from a portion of a frame acquisition, such as canceling an acquisition early, may trigger this scenario.
OverrideMeasuredLissajousMirrorPhase = 1;  ManualLissajousFastMirrorPhase = 453*1;  ManualLissajousSlowMirrorPhase = 640*1;
...
elseif Auto_Rotation %Take a single image
    samplesPerRecord = AcqParameters.ShotsPerMirrorPeriodGalvoRes*AcqParameters.HPixels;
    ...
    AcqParameters.Auto_Rotation = 1;
       if mod((Rotation_End_Point-Rotation_Start_Point),Rotation_Inteval)~=0
           fprintf('Error! Check Rotation end - start\n')
%            break
       end
       Original_FilePath = AcqParameters.FilePath;
       if Rotation_Start_Point<Rotation_End_Point
           flag = 0;
       else
           flag = 1;
           tempa = Rotation_Start_Point;
           tempb = Rotation_End_Point;
           Rotation_Start_Point = tempb;
           Rotation_End_Point = tempa;
       end
       count = 0;
       
       for quartz_inc = Rotation_Start_Point3:Rotation_Inteval3:Rotation_End_Point3
           movementParamaters = strcat(num2str(Rotation_Stage_Position3),'PA',num2str(quartz_inc));
           serialOne=serial(Rotation_Stage_Port, 'BaudRate', 19200, 'Terminator','CR/LF');
           fopen(serialOne);
           fprintf(serialOne,movementParamaters);
           fclose(serialOne);
           pause(Pause_Time3);
           ...
           if quartz_inc < 10
               Qz_folder_name = strcat('QuartzDegree','0',num2str(quartz_inc),'\');
           else
               Qz_folder_name = strcat('QuartzDegree',num2str(quartz_inc),'\');
           end
           
           for j = Rotation_Start_Point2:Rotation_Inteval2:Rotation_End_Point2
                movementParamaters = strcat(num2str(Rotation_Stage_Position2),'PA',num2str(j));
                serialOne=serial(Rotation_Stage_Port, 'BaudRate', 19200, 'Terminator','CR/LF');
                fopen(serialOne);
                fprintf(serialOne,movementParamaters);
                fclose(serialOne);
                pause(Pause_Time2);
                if Rotation_Inteval>0
                    if mod(count,2)==0
                        inner_rotate = Rotation_Start_Point:Rotation_Inteval:Rotation_End_Point;
                    else
                        inner_rotate = Rotation_End_Point:-Rotation_Inteval:Rotation_Start_Point;
                    end
                else
                    if mod(count,2)==0
                        inner_rotate = Rotation_Start_Point:-Rotation_Inteval:Rotation_End_Point;
                ...
                count = count+1;
                if j >=100
                    Inner_folder_name = strcat('PolarizerDegree',num2str(j),'\');
                elseif j >= 10
                    Inner_folder_name = strcat('PolarizerDegree','0',num2str(j),'\');
                else
                    Inner_folder_name = strcat('PolarizerDegree','00',num2str(j),'\');
                end
                ...
                for i = inner_rotate
                    ...
                    if Deg >= 100
                        AcqParameters.FilePath = strcat(Original_FilePath,Qz_folder_name,Inner_folder_name,num2str(Deg),'Degree');
                    elseif Deg >=10
                        AcqParameters.FilePath = strcat(Original_FilePath,Qz_folder_name,Inner_folder_name,'0',num2str(Deg),'Degree');
                    else 
                        AcqParameters.FilePath = strcat(Original_FilePath,Qz_folder_name,Inner_folder_name,'00',num2str(Deg),'Degree');
                    end

                    mkdir(AcqParameters.FilePath)
                    movementParamaters = strcat(num2str(Rotation_Stage_Position),'PA',num2str(Deg));
                    serialOne=serial(Rotation_Stage_Port, 'BaudRate', 19200, 'Terminator','CR/LF');
                    fopen(serialOne);
                    fprintf(serialOne,movementParamaters);
                    fprintf('\nMoving Rotation_Stage to %u Degree\n', Deg);
                    fclose(serialOne);
                    pause(Pause_Time)
                    acquireData(boardCount, boardHandleArray,AcqParameters)
                end
...
movementParamaters = strcat(num2str(Rotation_Stage_Position2),'PA',num2str(Rotation_Start_Point2));
serialOne=serial(Rotation_Stage_Port, 'BaudRate', 19200, 'Terminator','CR/LF');
fopen(serialOne);
fprintf(serialOne,movementParamaters);
fclose(serialOne);
