...
AcqParameters.FilePath = 'D:\Changqin\20200817\new_objective_zscan_BaTiO3_1um_uRA\';  %If this path is not in purple, something is wrong!
...
OverrideMeasuredGalvoResMirrorPhase = 1;           ManualGalvoResMirrorPhase = 2040*1; %-2290 %-2550 %-2589
       ...
       if Rotation_Start_Point<Rotation_End_Point
           ...
           Rotation_End_Point = tempa;
            ...
            for i = Rotation_Start_Point:Rotation_Inteval:Rotation_End_Point
                ...
                if Deg >= 100
                    AcqParameters.FilePath = strcat(Original_FilePath,num2str(Deg),'z');
                elseif Deg >=10
                    AcqParameters.FilePath = strcat(Original_FilePath,'0',num2str(Deg),'z');
                else 
                    AcqParameters.FilePath = strcat(Original_FilePath,'00',num2str(Deg),'z');
                end
                
%                 mkdir(AcqParameters.FilePath)
                movementParamaters = strcat('U',' 10'); %60 %% IMPORTANT: there should be a space befor the number!%%
                serialOne=serial(Rotation_Stage_Port, 'BaudRate', 9600, 'Terminator','CR/LF');
                fopen(serialOne);
                fprintf(serialOne,movementParamaters);
                fprintf('\nMoving Rotation_Stage to %u z\n', Deg);
                fclose(serialOne);
                pause(Pause_Time)
                acquireData(boardCount, boardHandleArray,AcqParameters)
            end
