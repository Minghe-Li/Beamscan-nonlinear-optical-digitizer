...
AcqParameters.FilePath = 'D:\Changqin\20191219\Background_10x_z_scan_pinhole_uRA\';  %If this path is not in purple, something is wrong!
...
Rotation_End_Point = 200; 
Rotation_Inteval = 1;
Pause_Time = 0.5; %In sec;

% Moteried Stage Connection and Position Reset
% 100000 clicks = 3.90625mm for Function moverel_deviceunit()
...
%Channel A     
CaptureChannelA = 1;
ChannelACount = 0;     ChannelAThreshold = -3.5;     ChannelALambda = 0;
...
OverrideMeasuredGalvoResMirrorPhase = 1;           ManualGalvoResMirrorPhase = 1730*1; %-2290 %-2550 %-2589
...
%                 end
                if Deg >= 100
                    AcqParameters.FilePath = strcat(Original_FilePath,num2str(Deg),'x');
                elseif Deg >=10
                    AcqParameters.FilePath = strcat(Original_FilePath,'0',num2str(Deg),'x');
                else 
                    AcqParameters.FilePath = strcat(Original_FilePath,'00',num2str(Deg),'x');
                end
                
%                 mkdir(AcqParameters.FilePath)
%                 movementParamaters = strcat('U',' 30'); %60 %% IMPORTANT: there should be a space befor the number!%%
%                 serialOne=serial(Rotation_Stage_Port, 'BaudRate', 9600, 'Terminator','CR/LF');
%                 fopen(serialOne);
%                 fprintf(serialOne,movementParamaters);
%                 fprintf('\nMoving Rotation_Stage to %u x\n', Deg);
%                 fclose(serialOne);
                moverel_deviceunit(m1, 26)
                fprintf('\nMoving Rotation_Stage to %u x\n', Deg);
                pause(Pause_Time)
                acquireData(boardCount, boardHandleArray,AcqParameters)
            end
