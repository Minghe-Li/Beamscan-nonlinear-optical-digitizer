...
AcqParameters.FilePath = 'D:\Changqin\20190227\Z-cut-quartz-800nm-depo-QWP45-f1-pol_rot\';  %If this path is not in purple, something is wrong!
...
Rotation_Stage_Position = 2;
Rotation_Start_Point = 0; 
Rotation_End_Point = 180; 
Rotation_Inteval = 15;
Pause_Time = 6; %In sec;
...
AcqParameters.NumberOfFramesPerImage =  8;
...
OverrideMeasuredGalvoResMirrorPhase = 1;           ManualGalvoResMirrorPhase = 1700; %-2290 %-2550 %-2589
...
%        for j = 1:31
%             movementParamaters = strcat('1','PA',num2str((j-1)*2));
%             serialOne=serial(Rotation_Stage_Port, 'BaudRate', 19200, 'Terminator','CR/LF');
%             fopen(serialOne);
%             fprintf(serialOne,movementParamaters);
%             fclose(serialOne);
%             if j<=5
%                 Z_Cut_folder_Name = strcat('Z_Cut_Degree','0',num2str((j-1)*2),'\');
%             else
%                 Z_Cut_folder_Name = strcat('Z_Cut_Degree',num2str((j-1)*2),'\');
%             end
            
            for i = Rotation_Start_Point:Rotation_Inteval:Rotation_End_Point
                if ~flag
                    Deg = i;
                else
                ...
                if Deg >= 100
%                     AcqParameters.FilePath = strcat(Original_FilePath,Z_Cut_folder_Name,num2str(Deg),'Degree');
                    AcqParameters.FilePath = strcat(Original_FilePath,num2str(Deg),'Degree');
                elseif Deg >=10
%                     AcqParameters.FilePath = strcat(Original_FilePath,Z_Cut_folder_Name,'0',num2str(Deg),'Degree');
                    AcqParameters.FilePath = strcat(Original_FilePath,'0',num2str(Deg),'Degree');
                else 
%                     AcqParameters.FilePath = strcat(Original_FilePath,Z_Cut_folder_Name,'00',num2str(Deg),'Degree');
                    AcqParameters.FilePath = strcat(Original_FilePath,'00',num2str(Deg),'Degree');
                end
...
%             movementParamaters = strcat('2','PA','0');
%             serialOne=serial(Rotation_Stage_Port, 'BaudRate', 19200, 'Terminator','CR/LF');
%             fopen(serialOne);
%             fprintf(serialOne,movementParamaters);
%             fclose(serialOne);
