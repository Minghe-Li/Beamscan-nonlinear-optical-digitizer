...
AcqParameters.FilePath = 'F:\Max\20180219-stars\sample\';  %If this path is not in purple, something is wrong!
...
Rotation_Stage_Position2 = 3;
Rotation_Start_Point = 0; 
Rotation_End_Point = 180; 
Rotation_Inteval = 3;
Pause_Time = 2.2; %In sec;
...
AcqParameters.NumberOfFramesPerImage =  32;
...
OverrideMeasuredGalvoResMirrorPhase = 1;           ManualGalvoResMirrorPhase = 1620; %-2290 %-2550 %-2589
...
elseif Auto_Rotation %Take a single image
    ...
    AcqParameters.Auto_Rotation = 1;
       ...
       if Rotation_Start_Point>Rotation_End_Point
           Rotation_Inteval = -Rotation_Inteval;
       end
            for i = Rotation_Start_Point:Rotation_Inteval:Rotation_End_Point
                Deg = i; %+90 for coparallel; Stage 2, polarizer
                Deg2 = -i/2; % Stage 3, waveplate
               
                if Deg >= 100
                    AcqParameters.FilePath = strcat(Original_FilePath,num2str(Deg),'DegPol');
                elseif Deg >=10
                    AcqParameters.FilePath = strcat(Original_FilePath,'0',num2str(Deg),'DegPol');
                else 
                    AcqParameters.FilePath = strcat(Original_FilePath,'00',num2str(Deg),'DegPol');
                end
                ...
                fclose(serialOne);
                movementParamaters = strcat(num2str(Rotation_Stage_Position2),'PA',num2str(Deg2));
                serialTwo=serial(Rotation_Stage_Port, 'BaudRate', 19200, 'Terminator','CR/LF');
                fopen(serialTwo);
                fprintf(serialTwo,movementParamaters);
                fprintf('\nMoving Rotation_Stage to %u Degree\n', Deg2);
                fclose(serialTwo);
                pause(Pause_Time)
                acquireData(boardCount, boardHandleArray,AcqParameters)
